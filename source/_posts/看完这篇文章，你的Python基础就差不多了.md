...
<pre class="hljs bash">        `$ sudo rm -r /opt/pycharm-2016.3.1/
` 
      </pre>
...
<pre class="hljs python">        `
          ...
          print(num) print(args) print(kwargs) demo(
...
![](https://i.loli.net/2018/12/17/5c177a068677f.jpg) 第二部分 面向对象
...
<pre class="hljs python">        `
            ...
            (self, name):
          print(
          "初始化方法 %s" % name) self.name = name ... tom = Cat(
          "Tom") ... lazy_cat = Cat(
          "大懒猫") ...
` 
      </pre>
...
![](https://i.loli.net/2018/12/17/5c177a070cba0.jpg) 私有属性和私有方法
...
![](https://i.loli.net/2018/12/17/5c177a0752e16.jpg) 多态示例
...
多个对象的方法， **在内存中只有一份**，在调用方法时，需要把 **对象的引用**传递到方法内部

![](https://i.loli.net/2018/12/17/5c177a085b9cd.jpg) 各个不同的属性，独一份的方法
...
![](https://i.loli.net/2018/12/17/5c177a200aa7f.jpg) 类的结构
...
<pre class="hljs python">        `
          ...
          print(
          "现在创建了 %d 个工具" % Tool.count)
` 
      </pre>

##### 属性的获取机制

在 Python 中 属性的获取 存在一个 **向上查找**机制

![](https://i.loli.net/2018/12/17/5c177a2088acc.jpg)

因此，要 **访问**类属性有两种方式：

*   `类名.类属性`
*   `对象.类属性` （不推荐，因为如果使用 `对象.类属性 = 值` 赋值语句，只会 **给对象添加一个属性**，而 **不会影响到类属性的值**）
...
*   注意： `__new__` 是一个 **静态方法**，在调用时需要 **主动传递** `cls` 参数

![](https://i.loli.net/2018/12/17/5c177a20a18fc.jpg)
...
<pre class="hljs python">        `
          ...
          return super().__new__(cls)
            ...
            (self):
          print(
          "初始化音乐播放对象") player = MusicPlayer() print(player)
...
![](https://i.loli.net/2018/12/17/5c177a20b9cea.jpg)
...
<pre class="hljs python">        `
          ...
          if cls.instance
          is
          None: cls.instance = super().__new__(cls)
...
<pre class="hljs python">        `
          ...
          if
          not MusicPlayer.init_flag: print(
          "初始化音乐播放器") MusicPlayer.init_flag =
          True
...
3、在 `__init__`方法中定义类的属性时，如果 不知道设置什么初始值，可以设置为 `None`）： `None` 关键字 表示 什么都没有，表示一个 **空对象**，没有方法和属性，是一个特殊的 **常量**。可以将 None 赋值给任何一个 **变量**。
...
<pre class="hljs python">        `__import__(
          'os').system(
          'ls')
